...
class RM_SS_mono_e14(Scheduler):
    ...
    def _change_speed(self, job):
        ...
        if job is not None:
            ...
            bdiff = job.task.data["dvs"]["bp"] - job.task.data["dvs"]["b"]
            #if isclose(self.min_slack_s, bdiff) or self.min_slack_s > bdiff:
            if self.min_slack_s > job.task.data["dvs"]["bp"] - job.task.data["dvs"]["b"]:
