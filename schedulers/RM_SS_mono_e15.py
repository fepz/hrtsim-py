...
class RM_SS_mono_e15(Scheduler):
    ...
    def _change_speed_new(self, job):
        if job is not None:
            self._cpu.set_lvl(self.lvl_tup[1][6])
            if job.task.wcet + job.task.data["dvs"]["bp"] > job.task.wcet + self.min_slack_s \
                    or job.task.wcet + job.task.data["dvs"]["bp"] > job.task.wcet + self.min_slack:
                a = 1
                self._cpu.set_lvl(self.lvl_tup[0][6])
                job.task.data["dvs"]["wb"] = "b"
            else:
                self.min_slack_s -= job.task.data["dvs"]["bp"]
