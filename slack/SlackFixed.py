...
def get_slack(task, task_list, tc):
    ...
    if task.identifier == 1:
        return {"slack": task.data["ss"]["di"] - tc - task.data["R"], "ttma": task.data["ss"]["di"], "cc": ceil.counter, "interval_length": 0, "slack_calcs": slack_calcs}
    ...
    if (htask.data["ss"]["di"] + htask.wcet >= task.data["ss"]["di"]) and (task.data["ss"]["di"] >= htask.data["ss"]["ttma"]):
        return {"slack": htask.data["ss"]["slack"] - task.wcet, "ttma": htask.data["ss"]["ttma"], "cc": ceil.counter, "interval_length": 0, "slack_calcs": slack_calcs}
    ...
    return {"slack": kmax, "ttma": tmax, "cc": ceil.counter + floor.counter,
            "interval_length": task.data["ss"]["di"] - intervalo, "slack_calcs": slack_calcs}
